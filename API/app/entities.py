...
def dist_maker_df(df: pd.DataFrame):
    ...
    for place, coord in tqdm(dists.items()):
        if place == "metro_dist":
            df[place] = [
                dist(c1, c2)
                for c1, c2 in df[["coord_house", "coords"]].itertuples(index=False)
            ]
        else:
...
class Coord_imp(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        X_copy = X.copy()
        X_copy["coord_house"] = tuple(
            zip(X_copy["latitude"].astype(float), X_copy["longitude"].astype(float))
        )
...
class Metro_Names(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        ...
        X_copy = X.copy()
        X_copy["metro_station"] = [
            close(coords) if name == "Нет данных" else name
            for coords, name in X_copy[["coord_house", "metro"]].itertuples(index=False)
        ]
...
class Some_transformations(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        ...
        cur = {"rub": 1, "rur": 1, "usd": usd, "eur": eur, "euro": eur}

        X_copy["currency"] = X_copy["currency"].map(cur)
        X_copy["price"] = X_copy["price"].astype(float) * X_copy["currency"].astype(
            float
        )
...
class LargeImputers(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        ...
        X_copy["room_count"] = [
            rooms(area)
            if np.isnan(room) or room in [0, "0", "False", False, "Нет данных"]
            else room
            for area, room in X_copy[["total_area", "room_count"]].itertuples(
                index=False
            )
        ]
        ...
        X_copy["build_year"] = [
            year_inp(station)
            if pd.isna(year) or year <= 1500 or year in ["Нет данных ", 0, False]
            else year
            for year, station in X_copy[["build_year", "metro_m"]].itertuples(
                index=False
            )
        ]
...
class Material_IMP(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        ...
        X_copy["material_type"] = [
            typer(year)
            if mtype in [0, "0", "False", False, "Нет данных"] or pd.isna(mtype)
            else mtype
            for year, mtype in X_copy[["build_year", "material_type"]].itertuples(
                index=False
            )
        ]
...
class Okrug_Normalizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        ...
        X_copy.okrug = [
            func(station) if "АО" not in distr else distr
            for station, distr in tqdm(
                X_copy[["metro_m", "okrug"]].itertuples(index=False)
            )
        ]
        return X_copy
...
class Ceiler(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None):
        ...
        for year, okrug, maty, he, rep, price, area in X_copy[
            [
                "build_year",
                "okrug",
                "material_type",
...
class Preprocessor:
    def __init__(self, path_or_DF):
        ...
        path = "/Users/artemmaslov/Documents/Elbrus/Estate_price_estimator/API/funcions/data"
        metro = pd.read_pickle(path + "/metro.pkl")
        metro_year = pd.read_pickle(path + "/metro_year.pkl")
        metro_okrug = pd.read_pickle(path + "/metro_okrug.pkl")
        RT = pd.read_pickle(path + "/okrug_to_repair.pkl")
        CE = pd.read_pickle(path + "/okgug_ceil_repair.pkl")

        if type(path_or_DF) == str:
            self.path = path_or_DF
            self.data = (
                pd.read_csv(self.path).drop_duplicates(subset=["id"]).set_index("id")
            )
        else:
    ...
    def transform(self):
        ...
        self.data_pre = self.data_pre.dropna(subset=["coords"]).drop(
            ["currency", "coords"], axis=1
        )
...
class Model:
    ...
    def fit(self):
        X_train, X_test, y_train, self.y_test = train_test_split(
            self.X_, self.Y_, test_size=self.test_size, random_state=42
        )
        ...
        X_[
            ["latitude", "longitude", "total_area", "moskva_city_dist", "kremlin_dist"]
        ] = X_[
            ["latitude", "longitude", "total_area", "moskva_city_dist", "kremlin_dist"]
        ].astype(
            float
